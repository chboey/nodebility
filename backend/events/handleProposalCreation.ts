import {
  ...
  import { deployProposalContract } from '../scripts/deploy';
  ...
  async function createProposal(parameters: any) {
    ...
    const votingTimerHours = days * 24;
  
    //contract calls.
    const contractAddr = await deployProposalContract(
      days,
      parameters.requestedTokenAmount,
      topicId.toString()
    );
    ...
    const proposal = {
      presetFields: [
        ...
        {
          name: 'field4',
          title: 'requested_token_amount',
          value: parameters.requestedTokenAmount,
          readonly: false,
        }, // agent fill this in
        {
          name: 'field5',
